...
# are displayed on a red background and labeled with the correct (left)
# and recognised digit (right.
def tf_format_mnist_images(X, Y, Y_, n=100, lines=10):
    ...
    everything_incorrect_first = tf.concat(
        [
            incorrectly_recognised_indices,
            correctly_recognised_indices],
        0)  # images reordered with indeces of unrecognised images first
    # compute first 100 only - no space to display more anyway
    ...
    # computed digits to be printed on the images
    ...
    superimposed_digits = tf.where(
        correct_prediction_s,
        tf.zeros_like(correct_tags),
        correct_tags +
        computed_tags)  # only pring the correct and computed digits on unrecognised images
    correct_bkg = tf.reshape(
        tf.tile([1.3, 1.3, 1.3], [28 * 28]), [1, 28, 28, 3])  # white background
    incorrect_bkg = tf.reshape(
        tf.tile([1.3, 1.0, 1.0], [28 * 28]), [1, 28, 28, 3])  # red background
    # pick either the red or the white background depending on recognised
    # status
    recognised_bkg = tf.gather(tf.concat(
        [incorrect_bkg, correct_bkg], 0), tf.cast(correct_prediction_s, tf.int32))
    ...
    for imslice in range(lines):
        Islices.append(tf.concat(tf.unstack(
            tf.slice(I, [imslice * n // lines, 0, 0, 0], [n // lines, 28, 28, 3])), 1))
...
def _display_time_histogram(ax, xdata, ydata, color):
    ...
    for i in range(int(math.ceil(HISTOGRAM_BUCKETS / 2.0))):
        ax.fill_between(xdata, ydata[:, midl -
                                     i], ydata[:, midh +
                                               1 +
                                               i], facecolor=color, alpha=1.6 /
                        HISTOGRAM_BUCKETS)
...
class MnistDataVis:
    ...
    def __init__(
            ...
            title5=None,
            title6=None,
            histogram4colornum=None,
            histogram5colornum=None,
            dpi=70):
        ...
        # hack...
        ax6.set_yticks([0, 280 - 4 * 56, 280 - 3 * 56,
                        280 - 2 * 56, 280 - 56, 280])
        ...
        def _key_event_handler(event):
            ...
            if fignum <= 6:
                ...
                fig.axes[fignum -
                         1].change_geometry(toggles[keycode][1], toggles[keycode][2], 1)
                ...
                fig.axes[fignum %
                         10 -
                         1].change_geometry(toggles[keycode][1], toggles[keycode][2], 2)
                ax6.set_aspect(1.0)  # special case for test digits
            elif fignum == 123456:
                for i in range(6):
                    fig.axes[i].set_visible(True)
                    fig.axes[i].change_geometry(
                        toggles[keycode][1], toggles[keycode][2], i + 1)
                ax6.set_aspect(1.0)  # special case for test digits
    ...
    def append_data_histograms(
        ...
        self.w3 = np.concatenate(
            (self.w3, np.expand_dims(
                probability_distribution(datavect1), 0)))
        datavect2.sort()
        self.b3 = np.concatenate(
            (self.b3, np.expand_dims(
                probability_distribution(datavect2), 0)))
        self._update_xmax(x)
    ...
    def animate(
        ...
        def animate_step(i):
            if (i == iterations // train_data_update_freq):  # last iteration
                ...
                for k in range(train_data_update_freq):
                    ...
                    request_test_data_update = (
                        n %
                        test_data_update_freq == 0) and (
                        n > 0 or one_test_at_start)
                    if more_tests_at_start and n < test_data_update_freq:
                        request_test_data_update = request_data_update
                    compute_step(
                        n, request_test_data_update, request_data_update)
        ...
        self._animation = animation.FuncAnimation(
            ...
            int(
                iterations //
                train_data_update_freq +
                1),
            init_func=self._mlp_init_func,
            interval=16,
            repeat=False,
            blit=False)
